...
def test(net, test_dl, classes, plot_path="", verbose=True, device="cpu"):
    net = net.to(device)
    net.eval()
    criterion = nn.CrossEntropyLoss()
    tqdm_train_iterator = tqdm(
    ...
    for idx, (data, target) in tqdm_train_iterator:
        ...
        tqdm_train_iterator.set_postfix(
            avg_test_acc=f"{test_running_acc/(idx+1):0.4f}",
    ...
    sns.heatmap(
        cfm, annot=True, cmap="Blues", fmt="d", xticklabels=classes, yticklabels=classes
    )
    plt.title("Confusion Matrix")
    plt.savefig(f"plots/cm_{plot_path}.png", format="png", dpi=300)
